...
class Construction(AnchoredObject):
    ...
    def build(cls, _id, _cdict, tmr_anchor=None) -> "Construction":
        ...
        if name in graph:
            ...
            c.set_raw_id(_id)
            c.set_raw_data(_cdict)
            for _k in _cdict:
                setattr(c, _k.lower().replace("-", "_"), _cdict[_k])
            c.set_space(space)
            c.__populate(_cdict)
            return c
    ...
    def set_raw_id(self, _id):
        self.raw_id = _id

    def set_raw_data(self, _data):
        self.raw_data = _data
...
def check_basic_compatability(input_combination: list) -> Tuple[bool, dict]:
    ...
    combination = {}
    flag = True  # TODO DELETE _ JUST FOR TESTING
    for c_id, c_raw in flat.items():
        if flag:  # TODO DELETE _ JUST FOR TESTING

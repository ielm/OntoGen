...
def api_generate():
    ...
    return_one_result = (
        "oneresult" in request.form and request.form["oneresult"].lower() == "true"
    )
    ...
    rargs_root_agent = (
        request.form["rargs-root-agent"] if "rargs-root-agent" in request.form else ""
    )
    rargs_root_beneficiary = (
        request.form["rargs-root-beneficiary"]
        if "rargs-root-beneficiary" in request.form
        else ""
    )
    rargs_frame_prefix = (
        request.form["rargs-prefix"] if "rargs-prefix" in request.form else ""
    )
    rargs_frame_delim = (
        request.form["rargs-delim"] if "rargs-delim" in request.form else ""
    )
    rargs_concept_prefix = (
        request.form["ont-space"] if "ont-space" in request.form else ""
    )
